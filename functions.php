...
if (isset($_POST['comment_posted'])) {
	...
	$sql = "INSERT INTO blog_comments (post_id, user_id, body, created_at, updated_at) VALUES (1, " . $user_id . ", '$comment_text', now(), null)";
	$result = mysqli_query($conn, $sql);
	// Query same comment from database to send back to be displayed
	$inserted_id = $conn->insert_id;
	$res = mysqli_query($db, "SELECT * FROM blog_comments WHERE id=$inserted_id");
	...
	if ($result) {
		$comment = "<div class='comment clearfix'>
					...
					<!-- reply form -->
					<form action='readpost.php' class='reply_form clearfix' id='comment_reply_form_" . $inserted_comment['id'] . "' data-id='" . $inserted_comment['id'] . "'>
...
if (isset($_POST['reply_posted'])) {
	...
	$sql = "INSERT INTO blog_replies (user_id, comment_id, body, created_at, updated_at) VALUES (" . $user_id . ", $comment_id, '$reply_text', now(), null)";
	$result = mysqli_query($conn, $sql);
	$inserted_id = $conn->insert_id;
	$res = mysqli_query($conn, "SELECT * FROM blog_replies WHERE id=$inserted_id");
